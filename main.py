...
def uploaded_file(filename):
    ...
    file = io.open(image_path, 'rb', buffering=0)
    result = query(file)
    print(result)
    try: 
      if result[0]['label'] == 'pothole':
        label = 'We found a pothole!'
        confidence = 'With ' + str(round(result[0]['score'], 2) * 100) + '%' + ' confidence.' 
      else:
    ...
    return render_template('results.html', confidences=confidence, labels=label, filename = filename)
